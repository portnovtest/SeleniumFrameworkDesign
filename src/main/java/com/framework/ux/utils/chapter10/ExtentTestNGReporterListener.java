...
public class ExtentTestNGReporterListener implements IReporter {
    ...
    private String seleniumRev = "3.141.59";
    ...
    private void init(ISuite suite){
        ...
        ExtentHtmlReporter htmlReporter = new ExtentHtmlReporter(Global_VARS.REPORT_PATH
                + suite.getName() + ".html");
    ...
    private void processTestResults(ISuiteResult r) throws Exception {
        ...
        Set<ITestResult> tests = new HashSet<>();
        ...
        if (tests.size() > 0){
            ...
            List<ITestResult> resultList = new LinkedList<>(tests);
            ...
            for (ITestResult result : resultList) {
                ...
                for (String group : result.getMethod().getGroups()) {
                    if (!group.isEmpty()){
                        ...
                        String testName = result.getMethod().getRealClass().getName().split("\\.")[size-1];
                    ...
                    if (!getTestParams(result).isEmpty()){
                        ...
                        if (result.getAttribute("testBitmap") != null){
                            test.log(Status.INFO,"SCREENSHOT",
                                    MediaEntityBuilder.createScreenCaptureFromPath(bitmapDir
                                            + result.getAttribute("testBitmap")).build());
                        }

                        test.log(Status.INFO,"STACKTRACE" + getStackTrace(result));
                    }
                }
            }
        }
    ...
    /**
     * getStackTrace method to retrieve stack trace
     *
     * @param result
     * @return String
     */
